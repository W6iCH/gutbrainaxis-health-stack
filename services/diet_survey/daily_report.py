...
# Portable default for the survey project directory: assume the standard
# install layout (services/diet_survey + services/sjtu_survey_pro as siblings)
# and allow an explicit override via SURVEY_APP_DIR.
...
def get_diet_stats():
    ...
    sys.path.insert(0, os.environ.get("DIET_APP_DIR", os.path.dirname(os.path.abspath(__file__))))
    ...
    # LLM queue status
    queue_dir = os.path.join(os.environ.get("DIET_APP_DIR", os.path.dirname(os.path.abspath(__file__))), ".llm_queue")
    ...
    # Email queue
    email_dir = os.path.join(os.environ.get("DIET_APP_DIR", os.path.dirname(os.path.abspath(__file__))), ".email_queue")
...
def send_report(html_body):
    ...
    try:
        ...
        conn.sendmail(SENDER, [ADMIN], msg.as_string())
