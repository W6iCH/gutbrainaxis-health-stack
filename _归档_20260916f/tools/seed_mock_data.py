...
def build_survey_row(svc_dir: str, idx: int, rnd: random.Random):
    ...
    for t, label_lists, p in (
        ...
        (SC._PSQI_C2A, ["少于 15 分钟", "16～30 分钟", "31～60 分钟", "超过 60 分钟"], 2),
    ...
    # ⚠️ 3.0-native-options：WHOQOL 单选题改为**问卷实测真实 label**
    # （旧脚本用臆造标签，原生引擎「未识别即报错」会直接失败）。
    _who_labels = {
        1: ["很差", "差", "一般", "好", "很好"],
        2: ["很不满意", "不满意", "一般", "满意", "很满意"],
        3: ["极妨碍", "比较妨碍", "有妨碍（一般）", "很少妨碍", "根本不妨碍"],
        4: ["极需要", "比较需要", "需要（一般）", "很少需要", "根本不需要"],
        5: ["根本没乐趣", "很少有乐趣", "有乐趣（一般）", "比较有乐趣", "极有乐趣"],
        6: ["根本没意义", "很少有意义", "有意义（一般）", "比较有意义", "极有意义"],
        7: ["根本不能", "很少能", "能（一般）", "比较能", "极能"],
        8: ["根本不安全", "很少安全", "安全（一般）", "比较安全", "极安全"],
        9: ["根本不好", "很少好", "好（一般）", "比较好", "极好"],
        10: ["根本没精力", "很少有精力", "有精力（一般）", "多数有精力", "完全有精力"],
        11: ["根本过不去", "很少过得去", "过得去（一般）", "比较过得去", "完全过得去"],
        12: ["根本不够用", "很少够用", "够用（一般）", "多数够用", "完全够用"],
        13: ["根本不齐备", "很少齐备", "齐备（一般）", "多数齐备", "完全齐备"],
        14: ["根本没机会", "很少有机会", "有机会（一般）", "多数有机会", "完全有机会"],
        15: ["很差", "差", "不好也不差", "好", "很好"],
        26: ["总是有消极感受", "经常有消极感受", "时有时无", "偶尔有消极感受", "没有消极感受"],
    }
    ...
    answers.append(choice(SC._WHOQOL_EXTRA[0],
                          ["有极大影响", "有比较大影响", "影响（一般）", "很少影响", "根本不影响"], 4))
...
def main() -> int:
    ...
    os.environ.setdefault("SCORING_VERSION", "3.0-native-options")
